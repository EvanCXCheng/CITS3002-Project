...
#Technically flexible, could be changed with user input
source = HostA
destination = HostB

input_size = int(sys.argv[1])
data = os.urandom(input_size) #Bytes
print(f"{source.Name}: Layer 4: Data received from Application Layer. Data size= {input_size}")
rdt = 0
i = 0
while i < input_size:
    segment = source.create_segment(min(MAX_SIZE, input_size - i), destination, data[i:i+MAX_SIZE], rdt)
    frame = Router1.receive_frame(segment)
    error_check = destination.receive_frame(frame, rdt)
    ...
    if error_check == "Old Ack":
        ...
        ack = destination.create_ack(input_size, source, 1 - rdt)
        ack = Router1.receive_frame(ack)
        error_check = source.receive_frame(ack, rdt)
        
    else:
        #Otherwise send a normal ack
        ack = destination.create_ack(input_size, source, rdt)
        ack = Router1.receive_frame(ack)
        error_check = source.receive_frame(ack, rdt)
